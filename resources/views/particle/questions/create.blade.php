...
@section('content')
    <div class="container-fluid mt--6">
        <div class="row">
            <div class="col">
                <div class="card">
                    <div class="card-header">Create New Particle Course Question</div>
                    <div class="card-body">
                        <form action="{{ route('particles.questions.create') }}" method="post">
                            @csrf
                            <div class="form-group">
                                <label for="particle_course_id">Particle Course</label>
                                <select name="particle_course_id" id="particle_course_id" class="form-control" required>
                                    <option value="">Select Particle Course</option>
                                    @foreach ($partCourses as $part)
                                        <option value="{{ $part->id }}">{{ $part->title }}</option>
                                    @endforeach
                                </select>
                                @error('particle_course_id')
