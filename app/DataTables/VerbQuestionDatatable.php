...
class VerbQuestionDatatable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($data) {
                $edit_url = route('verb-questions.edit', $data->id);
     ...
     */
    public function query(VerbCourseQuestion $model)
    {
        // DB::statement(DB::raw('set @rownum=0'));
        return $model->newQuery()
            // ->where('slug','!=','super-admin')
            ->select([
                'verb_course_questions.id',
                'verb_course_questions.code',
                'verb_course_questions.question_jpn',
    ...
    {
        return 'VerbQuestion_' . date('YmdHis');
    }
}
