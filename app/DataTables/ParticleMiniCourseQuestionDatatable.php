...
class ParticleMiniCourseQuestionDatatable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($data) {
                $edit_url = route('particle-mini-course-questions.edit', $data->id);
     ...
     */
    public function query(ParticleMiniCourseQuestion $model)
    {
        // DB::statement(DB::raw('set @rownum=0'));
        return $model->newQuery()
            // ->where('slug','!=','super-admin')
            ->select([
                'particle_mini_course_questions.id',
                'particle_mini_course_questions.code',
                'particle_mini_course_questions.question_jpn',
    ...
    {
        return 'ParticleMiniCourseQuestion_' . date('YmdHis');
    }
}
