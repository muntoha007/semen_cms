...
class VocabularyCourseQuestionDatatable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($data) {
                $edit_url = route('vocabulary-course-questions.edit', $data->id);
     ...
     */
    public function query(VocabularyCourseQuestion $model)
    {
        // DB::statement(DB::raw('set @rownum=0'));
        return $model->newQuery()
            // ->where('slug','!=','super-admin')
            ->select([
                'vocabulary_course_questions.id',
    ...
    {
        return 'VocabularyCourseQuestion_' . date('YmdHis');
    }
}
